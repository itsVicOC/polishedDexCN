...
const [rawPokemon, rawMoves, rawItems, rawAbilities, rawTrainers, rawLocations, rawLocationDetails, rawTrainerDetails, rawPokemonDetails, rawItemDetails, pokemonZh, movesZh, itemsZh, abilitiesZh, locationsZh, rawEvolutions, events, guides] = await Promise.all([
  ...
  'pokemon-zh', 'moves-zh', 'items-zh', 'abilities-zh', 'locations-zh', 'evolutions', 'events', 'guides',
].map(load))
...
const pokemon = rawPokemon.map(record => {
  ...
  return {
    id:record.dexNo,slug:record.id,name:rawLocalized(record.name,record.id),types:(form.types||firstVersion(record).plain?.types||[]).map(rawType),genus:record.dexEntry?.category||'Pokémon',description:record.dexEntry?.description||'',
    abilities:(form.abilities||[]).map((ability,index)=>({name:localized(ability.name,ability.id,abilitiesZh),role:abilityRoles[index]||`特性 ${index+1} · Ability ${index+1}`,effect:ability.description||''})),
